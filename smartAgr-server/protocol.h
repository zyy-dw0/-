...
数量表 t_amount
    arm_id INTEGER PRIMARY KEY, // 只一个网关，值为1
...
节点信息表  t_zigbee
    zigbee_id   char(20)    // 主键 节点id
    area_id     int         // 区域编号     默认为0表示此节点未添加到区域
    dev_type    int         // 节点设备类型 默认为0表示未知类型
    dev_status  int         // 设备状态     默认为0表示关(针对于执行器)

    create table t_zigbee(zigbee_id char(20) primary key, dev_type int default 0, area_id int default 0, dev_status int default 0);

设备信息表   t_device
    dev_type    int         // 主键     设备类型
    dev_name    varchar(30) // 设备名称 最多10个中文

    create table t_device(dev_type int primary key, dev_name varchar(30) not null);
